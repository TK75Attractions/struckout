...
pub trait Transport {
    async fn listen(&mut self,frame_tx: FrameSender,camera_loc_tx: CamaraLocSender);
}
...
pub struct UdpTransport {
    ...
    #[cfg(feature="no_std")]
    socket: embassy_net::udp::UdpSocket
}

impl Transport for UdpTransport {
    fn listen(&mut self,frame_tx: FrameSender,camera_loc_tx: CamaraLocSender) -> impl Future<Output = ()> {
        loop{
            self.socket.
        }
    }
...
impl UdpTransport {
    pub fn new()->std::io::Result<Self>{
        Ok(Self {  
            #[cfg(feature="std")]
            socket: std::net::UdpSocket::bind("0.0.0.0:5109")?,
            #[cfg(feature="no_std")]
            socket: embassy_net::udp::UdpSocker::bind("0.0.0.0:5109")?
        })
    }
}
